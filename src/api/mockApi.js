...
// ---------------------------------------------------------------------
const GradeKnobs = {
  '6': { intAbs:10, fracDen:[2,3,4,5,6], dec2:false, sci:{ dist:[50,200], time:[5,30], mass:[100,500], vol:[50,250], dv:[2,10] } },
  '7': { intAbs:15, fracDen:[2,3,4,5,6,8,9], dec2:true,  sci:{ dist:[100,500], time:[5,60], mass:[200,1000], vol:[100,500], dv:[3,15] } },
  '8': { intAbs:25, fracDen:[3,4,5,6,7,8,9,10,12], dec2:true, sci:{ dist:[200,1000], time:[10,120], mass:[500,2000], vol:[200,1000], dv:[5,20] } },
  '9': { intAbs:40, fracDen:[5,6,7,8,9,10,12,15], dec2:true, sci:{ dist:[500,3000], time:[10,180], mass:[1000,5000], vol:[500,2000], dv:[8,30] } },
  '10':{ intAbs:60, fracDen:[6,8,9,10,12,15,16], dec2:true, sci:{ dist:[1000,5000], time:[20,300], mass:[2000,10000], vol:[1000,4000], dv:[10,40] } },
};
...
function bFractions(rng,k){
  ...
  const num=a*d + c*b, den=b*d, g=gcd(num,den); const ans=`${num/g}/${den/g}`;
  return { stem:`Compute: ${a}/${b} + ${c}/${d}.`, answer:ans,
    altA:`Add ${a}/${b} and ${c}/${d}.`, altB:`What is ${a}/${b} plus ${c}/${d}?`,
...
function bDecimals(rng,k){
  ...
  return { stem:`Add the decimals: ${a} + ${b}.`, answer:sum,
    altA:`Find the sum of ${a} and ${b}.`, altB:`Compute ${a} + ${b}.`,
    rubric:`1 pt: align decimals; 1 pt: sum ${sum}.` };
}
function bIntegers(rng,k){
  const A=k.intAbs, a=Math.floor(rng()*(2*A+1))-A, b=Math.floor(rng()*(2*A+1))-A, add=rng()<0.5;
  const ans = add ? a+b : a-b;
  return { stem:`Evaluate: ${a} ${add?'+':'−'} ${b}.`, answer:String(ans),
    altA:`Compute the integer ${add?'sum':'difference'} of ${a} and ${b}.`,
    altB:`What is ${a} ${add?'+':'−'} ${b}?`, rubric:`1 pt: sign rules; 1 pt: final ${ans}.` };
}
function bGeometry(rng){
  ...
  return { stem:`A rectangle is ${w} by ${h} units. Find its ${area?'area':'perimeter'}.`, answer:ans,
...
function bAlgebra(rng,k){
  ...
  return { stem:`Solve for x: ${a}x + ${b} = ${c}.`, answer:String(x),
    altA:`Find x if ${a}x + ${b} = ${c}.`, altB:`Determine x: ${a}x + ${b} = ${c}.`,
...
// Science/Bio builders (Assessment)
function bPhysSpeed(rng,k){
  const d=ri(rng,k.sci.dist), t=ri(rng,k.sci.time), v=(d/t).toFixed(2);
  return { stem:`A body travels ${d} m in ${t} s. What is its average speed (m/s)?`, answer:String(v),
    altA:`Compute v for d=${d} m, t=${t} s.`, altB:`Find speed using v=d/t.`,
    rubric:`1 pt: v=d/t; 1 pt: ${d}/${t}=${v} m/s.` };
}
function bPhysAccel(rng,k){
  const dv=ri(rng,k.sci.dv), t=ri(rng,k.sci.time), a=(dv/t).toFixed(2);
  return { stem:`A cart increases its speed by ${dv} m/s in ${t} s. What is its average acceleration (m/s²)?`, answer:String(a),
    altA:`Compute a=Δv/t for Δv=${dv} m/s, t=${t} s.`, altB:`Find acceleration given change in speed ${dv} and time ${t}.`,
    rubric:`1 pt: a=Δv/t; 1 pt: ${dv}/${t}=${a} m/s².` };
}
function bChemDensity(rng,k){
  const m=ri(rng,k.sci.mass), V=ri(rng,k.sci.vol), rho=(m/V).toFixed(2);
  return { stem:`A sample has mass ${m} g and volume ${V} cm³. Find its density (g/cm³).`, answer:String(rho),
    altA:`Compute ρ=m/V for m=${m} g, V=${V} cm³.`, altB:`What is ρ if mass=${m} g and volume=${V} cm³?`,
    rubric:`1 pt: ρ=m/V; 1 pt: ${m}/${V}=${rho} g/cm³.` };
}
function bChemPercent(rng){
  const part=10+Math.floor(rng()*90), total=part+(10+Math.floor(rng()*90)), pct=((part/total)*100).toFixed(1);
  return { stem:`A solution has ${part} g of solute in ${total} g total mass. What is the percent composition of solute?`, answer:`${pct}%`,
    altA:`Compute % = (part/total)×100.`, altB:`Find percent by mass for ${part} of ${total}.`,
    rubric:`1 pt: %=(part/total)×100; 1 pt: ${(part/total*100).toFixed(1)}%.` };
}
function bBioPunnett(rng){
  const crosses=['Aa×Aa','Aa×aa','AA×aa','Aa×AA']; const c=pick(rng,crosses);
  let ans='100%'; if (c==='Aa×Aa') ans='75%'; if (c==='Aa×aa') ans='50%';
  return { stem:`For the monohybrid cross ${c} (A=dominant, a=recessive), what percent of offspring show the dominant phenotype?`, answer:ans,
    altA:`Predict dominant phenotype % for ${c}.`, altB:`Using a Punnett square, what % are dominant?`,
...
// ---------------------------------------------------------------------
// Grading batch generator (new, additive)
// ---------------------------------------------------------------------
const STUDENT_FIRST = ['Arjun','Meera','Kavya','Dev','Zoya','Irfan','Nina','Omar','Lila','Rohit','Tara','Isha','Ravi','Anaya','Kabir','Maya','Zain','Nikhil','Rhea','Veer'];
...
export async function generateGradingBatch(optsOrSeed, countMaybe) {
  ...
  // Tie expected mean score slightly to subject & grade
  const baseBySubject = { Math: 0, Science: -2, Biology: -1 };
  const subjectAdj = baseBySubject[subject] ?? 0;
  const gradeAdj = (Number(grade) - 7) * -1.5; // a bit harder at higher grades
...
export const api = {
  ...
  generateGradingBatch, // additive; Grading page can use this, or fall back to getGradingBatch()
};
